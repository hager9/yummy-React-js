  ...
  async function recipeProcessing() {
    ...
    for (let i = 0; i < mealDetailsMap.size; i++) {
      if (mealDetailsMap.get(`strIngredient${i}`)) {
        mealDetailsObject.ingredients.push(
          `${mealDetailsMap.get(`strMeasure${i}`)} ${mealDetailsMap.get(
            `strIngredient${i}`
          )}`
        );
      }
        ...
        <div className="row g-4 ps-md-5">
          <div className="col-md-4">
            <div className="meal-img overflow-hidden">
              ...
              />
              <h2 className="mt-3">{mealDetails?.strMeal}</h2>
            </div>
          </div>
            ...
            <div className="meal-details">
              ...
              <ul className="list-unstyled d-flex flex-wrap g-3">
                {mealDetails?.ingredients.map((ingredient, index) => (
                  <li key={index} className="alert alert-info p-1 m-2">
                    {ingredient}
                  </li>
                ))}
              </ul>
                ...
                <>
                  ...
                  <ul className="list-unstyled d-flex flex-wrap g-3">
                    {mealDetails?.strTags.split(",").map((tag, index) => (
                      <li key={index} className="alert alert-danger p-1 m-2">
                        {tag}
                      </li>
                    ))}
                  </ul>
